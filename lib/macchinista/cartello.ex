defmodule Macchinista.Cartello do
  ...
  def delete_checklist(checklist, _user) do
    Repo.transaction(fn ->
      checklist = Repo.delete(checklist)
      case checklist do
        {:ok, _checklist} = result ->
          result
        {:error, _changeset} ->
  ...
  @spec delete_quest(Task.t, User.t) ::
    {:ok, Task.t}
    | {:error, atom() | String.t}
  def delete_quest(task, _user) do
    Repo.transaction(fn ->
  ...
  def move_card_to_card_list(%Card{card_list: from, order: from_order} = card, to_order, destination, _user) do
    Repo.transaction(fn ->
      last_order =
        destination
  ...
  def reorder_card(%Card{parent: parent_id, order: from} = card, order, _user) do
    Repo.transaction(fn ->
      parent =
        parent_id
        |> Repo.get(Card)
  ...
  @spec reorder_checklist(Checklist.t, Checklist.order, User.t) ::
    {:ok, [Checklist.t]}
    | {:error, atom() | String.t}
  def reorder_checklist(%Checklist{order: from, card: card} = checklist, order, _user) do
    Repo.transaction(fn ->
      last_order =
        card
